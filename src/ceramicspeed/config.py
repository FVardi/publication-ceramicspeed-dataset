...
Usage
-----
    from ceramicspeed.config import load_config
    cfg = load_config()            # loads config.yaml in project root
...
# Default location: config.yaml in the project root (src/ceramicspeed/ → src/ → root)
