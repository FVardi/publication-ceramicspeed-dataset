...
The test rig is a custom-built bearing test stand at Aarhus University (Herning, Denmark). The rig is used for conducting experiments under conditions, where rotational speed, load, temperature, and lubricant amount can be controlled. \todo{Should there be more general motivation for this?}
...
\begin{description}
  ...
  \item[\textbf{Acoustic emission sensor:}] a Kistler Piezotron Type~8152C1 wideband piezoelectric AE sensor bonded to the bearing housing\todo{How is it mounted?}. Its output is conditioned by a Kistler Type~5125C AE Piezotron coupler, which powers the sensor and applies a Butterworth band-pass filter from \SI{50}{kHz} to \SI{1}{MHz} to the raw AE signal before it is digitised. This coupler band-pass, sets the \SI{1}{MHz} upper analysis limit used throughout the paper.
  \item[\textbf{Ultrasound sensor:}] a UE Systems Ultra-trak 750 sensor \todo{How is it mounted?} \todo{Check if the signal is actually heterodyned.}.
  \item[\textbf{Oscilloscope:}] an Agilent MSO2024, sampling the AE and US signals at a frequency of 12.5MHz.
\end{description}
