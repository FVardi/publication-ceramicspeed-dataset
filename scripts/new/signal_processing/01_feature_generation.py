...
if SENSORS:
    ...
    existing_path = NEW_DIR / "features.parquet"
...
df.to_parquet(NEW_DIR / "features.parquet", engine="pyarrow")
print(f"Saved: {NEW_DIR / 'features.parquet'}  ({len(df)} rows)")

if not SENSORS:
    metadata_df.to_parquet(NEW_DIR / "metadata.parquet", engine="pyarrow")
    print(f"Saved: {NEW_DIR / 'metadata.parquet'}  ({len(metadata_df)} rows)")

    signal_quality_df.to_parquet(NEW_DIR / "signal_quality.parquet", engine="pyarrow")
    print(f"Saved: {NEW_DIR / 'signal_quality.parquet'}  ({len(signal_quality_df)} rows)")
