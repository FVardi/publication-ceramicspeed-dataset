...
Each signal is cleaned (NaN/Inf interpolation, spike removal, clipping and saturation detection) before feature extraction. Because the UL sensor is a heterodyned probe with effective content below approximately 10 kHz, a sensor-specific pre-filter is applied to UL signals before feature computation, preventing out-of-band noise from biasing spectral features such as centre frequency and RMS frequency. In addition to broadband processing, physics-motivated bandpass-filtered variants are extracted: for AE, bands at 20–500 kHz, 500–1000 kHz, and 1000–2000 kHz; for UL, bands at 0–10 kHz and 10–20 kHz. From each signal, 26 features are computed comprising 12 time-domain statistics (including Hjorth mobility and complexity) and 14 frequency-domain statistics (spectral shape, flatness, kurtosis, normalised bandwidth). Feature selection applies a Spearman rank correlation threshold (|ρ| ≥ 0.1 and |r| ≥ 0.1) followed by a variance inflation factor and pairwise correlation filter, retaining approximately 36 AE and 9 UL features for modelling.

## Modelling

Three model families are benchmarked across three sensor configurations (AE only, UL only, AE + UL combined): Elastic Net, polynomial regression (degree 2), and LightGBM gradient boosting. Models are evaluated via repeated nested cross-validation (5 outer folds × 5 repeats, κ-stratified) on an 80% training split and evaluated on a held-out 20% test set. The best performance is achieved by LightGBM on AE features alone (CV estimate: R² ≈ 0.951, RMSE ≈ 0.078 κ-units). Fusing AE and UL features does not improve upon AE alone — a negative fusion result attributed to the limited independent κ information carried by the heterodyned acoustic channel relative to AE. Statistical testing (Wilcoxon signed-rank, Diebold-Mariano, bootstrap ΔRMSE CI) confirms the null result is not a power issue.
